...
import { LayoutDashboard, FileSearch, BarChart3, Users, LogOut } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    ...
    >
      {/* Logo */}
      <div style={{ padding: "22px 20px", borderBottom: "1px solid rgba(43,63,191,0.08)" }}>
        <span style={{ fontSize: 17, fontWeight: 700, color: "#1a1f3c", letterSpacing: "-0.04em" }}>
          Jobflow
        </span>
      </div>
      ...
      <nav style={{ padding: "12px 10px", flex: 1 }}>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <Icon size={16} style={{ opacity: isActive ? 1 : 0.3, color: isActive ? "#2B3FBF" : "#1a1f3c" }} />
      ...
      {/* User section */}
      <div style={{ padding: "14px 16px", borderTop: "1px solid rgba(43,63,191,0.08)", display: "flex", alignItems: "center", gap: 10 }}>
        {user?.picture ? (
          <img
        ...
        <button
          ...
          onClick={logout}
          title="Logout"
          style={{ background: "none", border: "none", cursor: "pointer", color: "#8892b0", padding: 4, flexShrink: 0 }}
        >
          <LogOut size={14} />
        </button>
      </div>
